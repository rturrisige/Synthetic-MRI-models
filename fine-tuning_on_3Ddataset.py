...
import os
import argparse
from data_utilities import Loader3D
from utilities import load_ACS_pretrained_model, Trainer, create_next_exp_folder
...
def finetuning(config, model, w_path, data_path, saver_path, all_results=None):
    ...
    val_files, val_labels = np.load(data_path + '/val_images.npy')[:100], np.load(data_path + '/val_labels.npy')[:100].squeeze(1)
    train_files, train_labels = np.load(data_path + '/train_images.npy')[:100], np.load(data_path + '/train_labels.npy')[:100].squeeze(1)

    train_data = Loader3D(train_files, train_labels, nchannels=config.nchannels)
    train_loader = DataLoader(train_data, sampler=None, batch_size=config.train_batch_size, num_workers=1,
                              drop_last=False, pin_memory=True, prefetch_factor=2)

    val_data = Loader3D(val_files, val_labels, nchannels=config.nchannels)
    ...
    # LOAD TESTING DATA
    test_files, test_labels = np.load(data_path + '/test_images.npy')[:100], np.load(data_path + '/test_labels.npy')[:100].squeeze(1)

    test_data = Loader3D(test_files, test_labels, nchannels=config.nchannels)
...
class TrainingConfiguration(object):
    def __init__(self, lr=0.0001, l1=0.0, l2=0.0, nc=1, nclasses=2):
        self.num_epochs = 2  # maximum number of iterations
